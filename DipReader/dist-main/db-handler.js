...
class DatabaseHandler {
    ...
    _ensureDbPath() {
        if (!this.dbPath) {
            this.dbPath = path.join(electron_1.app.getPath('userData'), 'databases');
    ...
    async openOrCreateDatabase(dipUUID) {
        ...
        try {
            const sqliteVss = await Promise.resolve().then(() => __importStar(require('sqlite-vss')));
    ...
    exportDatabase(exportPath) {
        ...
        try {
            ...
            const targetPath = exportPath || path.join(electron_1.app.getPath('downloads'), `${this.currentDipUUID}.sqlite3`);
