...
class IndexerMain {
  private db: typeof DatabaseHandler;
  private dipRootPath: string;
  ...
  async parseDipIndexXml(xmlContent: string): Promise<void> {
    ...
    try {
      ...
      const processUUIDs = Array.from(xmlDoc.getElementsByTagName('AiP')).map(aip => aip.getAttribute('uuid')).filter((uuid): uuid is string => uuid !== null);
  ...
  insertMetadata(key: string, value: string, documentId: number, metaType: string = 'string', fileId: number | null = null): void {
    if (!this.db) return;
  ...
  extractOptionalMetadata(xmlDoc: any, documentId: number, tagName: string, metaType: string = 'string'): void {
  ...
  processImpronte(xmlDoc: any, documentId: number): void {
    ...
    const improntaMap = new Map<string, { impronta: string; algoritmo: string }>(); // UUID → { impronta, algoritmo }
