...
export class SearchService {
  ...
  async searchSemantic(query: string | number[]): Promise<{id: number, score: number}[]> {
    ...
    try {
      const results = await window.electronAPI.ai.search(query);
      return results;
    } catch (error) {
  ...
  async indexDocument(docId: number, metadataCombinedText: string) {
    if (this.isAiReady) {
      try {
        await window.electronAPI.ai.index(docId, metadataCombinedText);
      } catch (error) {
  ...
  async getEmbeddingDebug(text: string): Promise<number[]> {
    ...
    try {
      const embedding = await window.electronAPI.ai.generateEmbedding(text);
      return embedding;
    } catch (error) {
