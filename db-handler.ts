// db-handler.ts - Main process database handler
...
class DatabaseHandler {
   ...
   */
  async openOrCreateDatabase(dipUUID: string): Promise<OpenDatabaseResult> {
    ...
    try {
      const sqliteVss: any = await import('sqlite-vss');
      ...
      const stripExtension = (p: string): string => p.replace(/\.(so|dylib|dll)$/, '');
   ...
   */
  createSchema(): void {
    const schemaPath = path.join(__dirname, 'assets', 'schema.sql');

    if (!fs.existsSync(schemaPath)) {
      console.error('[DB Handler] Schema file not found:', schemaPath);
      return;
    }
  ...
  getAllVectors(): Array<{ id: number }> {
    if (!this.db) return [];

    try {
      let rows: VectorRow[];
      if (this.vssEnabled) {
        rows = this.db.prepare('SELECT rowid as doc_id FROM vss_documents').all() as VectorRow[];
      } else {
        rows = this.db.prepare('SELECT doc_id FROM document_vectors').all() as VectorRow[];
      }
   ...
   */
  searchVectors(queryVector: Float32Array, limit: number = 20): Array<{ id: number; score: number }> {
    if (!this.db) return [];

    try {
      // Debug: Check how many vectors exist
      const vectorCount = this.vssEnabled
        ? (this.db.prepare('SELECT COUNT(*) as count FROM vss_documents').get() as CountResult).count
        : (this.db.prepare('SELECT COUNT(*) as count FROM document_vectors').get() as CountResult).count;
      ...
      if (this.vssEnabled) {
        ...
        const results = this.db.prepare(`
                  ...
                  LIMIT ?
              `).all(vectorJson, limit) as VectorSearchResult[];
   ...
   * Delete a database
   */
  deleteDatabase(dipUUID: string): { success: boolean; error?: string } {
    try {
      const dbFileName = `${dipUUID}.sqlite3`;
      const fullPath = path.join(this.dbPath!, dbFileName);
   ...
   */
  exportDatabase(exportPath?: string): { success: boolean; path?: string; error?: string } {
  ...
  getDatabaseInfo(): DatabaseInfo {
    ...
    try {
      const fileCount = this.db.prepare('SELECT COUNT(*) as count FROM file').get() as CountResult;
      const documentCount = this.db.prepare('SELECT COUNT(*) as count FROM document').get() as CountResult;

      // Get vector count
      let vectorCount = 0;
      try {
        if (this.vssEnabled) {
          const result = this.db.prepare('SELECT COUNT(*) as count FROM vss_documents').get() as CountResult;
          vectorCount = result.count;
        } else {
          const result = this.db.prepare('SELECT COUNT(*) as count FROM document_vectors').get() as CountResult;
          vectorCount = result.count;
        }
      } catch (e) {
      ...
      return { open: true, dipUUID: this.currentDipUUID, error: (e as Error).message };
    }
  }
